...
def split_sentence(sentence, num_parts, word_limit=20, index=-1, retry_attempt=0):
    ...
    # 在 retry 时向 prompt 追加明确的重试标记，避免用空格 hack 绕过缓存
    retry_suffix = f"\n[Retry attempt {retry_attempt}]" if retry_attempt > 0 else ""
    response_data = ask_gpt(split_prompt + retry_suffix, resp_type='json', valid_def=valid_split, log_title='split_by_meaning')
    choice = response_data["choice"]
