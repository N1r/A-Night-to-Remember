...
def demucs_audio():
    ...
    try:
        ...
        raise ImportError("Demucs requires 'torch' and 'demucs' to be installed. Please install them or disable demucs in config.yaml.")
    ...
    class PreloadedSeparator(Separator):
        def __init__(self, model: BagOfModels, shifts: int = 1, overlap: float = 0.25,
            ...
            device = "cuda" if is_cuda_available() else "mps" if (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()) else "cpu"
