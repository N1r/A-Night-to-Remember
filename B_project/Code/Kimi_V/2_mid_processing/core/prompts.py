...
def get_summary_prompt(source_content, custom_terms_json=None):
    ...
    domain_prompts = domain.get("prompts", {})
    role = domain_prompts.get("summary", {}).get("role", f"You are a video translation expert and terminology consultant, specializing in {src_lang} comprehension and {tgt_lang} expression optimization.")
    task_desc = domain_prompts.get("summary", {}).get("task", f"For the provided {src_lang} video text:")
